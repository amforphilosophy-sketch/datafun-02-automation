...
SUBJECT_LIST: Final[list[str]] = ["algebra", "geometry", "statistics"]
...
def create_files_from_list() -> None:
    ...
    LOG.info(f"Pet list: {SUBJECT_LIST}")

    for subject_name in SUBJECT_LIST:
        filename: str = f"case_{subject_name}.txt"
        path: Path = PROCESSED_DIR / filename
        content: str = f"Pet data for: '{subject_name}'\n"
        write_text_file(path=path, content=content)
...
def create_files_using_list_comprehension() -> None:
    ...
    LOG.info(f"Original list: {SUBJECT_LIST}")
    favorite_list: list[str] = [f"{prefix}{name}" for name in SUBJECT_LIST]
